...
function C:Update(addonName)
	...
	if not self.tree then
		C:GetTreeGroupFramesFrame(self)
	end
...
local function UpdateRightSide(self)
	local elements_row = floor((self:GetWidth()-dims)/180)
	...
	if self.last_elements_row then
		if elements_row == self.last_elements_row then return end		
	end
	
	self.last_elements_row = elements_row
	
--	print("UpdateRightSide")
...
function C:GetRightGroupFramesFrame(self, o)
	...
	for i=1, #s do
		local opts = datas.args[s[i].name]
		...
		if prototype ~= "group" then
			...
			panel.elements[index]:Update(panel.buttonParent, opts)
			panel.elements[index]:ClearAllPoints()
			
			if frames == 0 then
				frames = frames + 1
				panel.elements[index]:SetPoint("TOPLEFT", panel.buttonParent, "TOPLEFT")
				...
				if height == "full" then
					...
					panel.elements[index]:SetPoint("BOTTOM", panel.buttonParent, "BOTTOM")
				...
				elseif frames >= elements_row then
					...
					frames = frames + 1
					panel.elements[index]:SetPoint("TOPLEFT", panel.elements[index-1], "TOPRIGHT")
					if currentheight < panel.elements[index]:GetHeight() then
						currentheight = panel.elements[index]:GetHeight()
					end
				end
