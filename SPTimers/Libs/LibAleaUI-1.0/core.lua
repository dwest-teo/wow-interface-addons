...
do
	...
	local function Slash(addon, command, func)
		...
		if not db[addon] then 
			db[addon] = 0 		
			SlashCmdList[addon] = func
		end
		
		db[addon] = db[addon] + 1	
		_G[slashtag..addon..db[addon]] = command	
	end
...
do
	...
	local cradius = GetMiniMapButtonsRad and GetMiniMapButtonsRad() or 80
	...
	function updatePosition(button)
		...
		if quadTable[q] then
			x, y = x*cradius, y*cradius
		else
			local diagRadius = math.sqrt(2*(cradius)^2)-10
			x = math.max(-cradius, math.min(x*diagRadius, cradius))
			y = math.max(-cradius, math.min(y*diagRadius, cradius))
		end
	...
	local function MinimapButtonCreate(texture, addon)
		...
		cradius = GetMiniMapButtonsRad and GetMiniMapButtonsRad() or 80
...
do
	...
	function ALEAUI_NewDB(variable, default, usedefault)
		...
		if _G[variable].enableDualProfile == nil then
			_G[variable].enableDualProfile = false
		end	
		...
		if not _G[variable].profileKeys[profileOwner][_G[variable].enableDualProfile and activespec or 1] then		
			_G[variable].profileKeys[profileOwner][_G[variable].enableDualProfile and activespec or 1] = DEFAULT_NAME
		end
		
		if not _G[variable].profiles[_G[variable].profileKeys[profileOwner][_G[variable].enableDualProfile and activespec or 1]] then
			_G[variable].profileKeys[profileOwner][_G[variable].enableDualProfile and activespec or 1] = DEFAULT_NAME
		end
		
		local instance = GetInstance(variable, default, _G[variable].profileKeys[profileOwner][_G[variable].enableDualProfile and activespec or 1])
		instance:Refresh()
...
function h:ACTIVE_TALENT_GROUP_CHANGED(event)
	...
	if activespec ~= lastactivespec then
		for db, instance in pairs(db_variables) do
			...
			if _G[db].enableDualProfile then
...
local function GetDeleteInterator(db)
	return function()
		...
		local profile2 = _G[db].enableDualProfile and _G[db].profileKeys[profileOwner][2] or profile1

		for profileKey in pairs(_G[db].profiles) do			
			if profile1 ~= profileKey and profile2 ~= profileKey then
				t[profileKey] = profileKey
			end
		end
...
--	print("T", _G[db].profileKeys[profileOwner][spec], value)
	if _G[db].profileKeys[profileOwner][spec] ~= value then
		...
		if ( _G[db].enableDualProfile and lastactivespec or 1 ) == spec then
			instance:Save()
			instance:Fire(PROFILE_CHANGED)
		end
	end	
...
function ALEAUI_GetProfileOptions(db, singleSpec)
	...
	f.args.enabledual = {
		...
		func = function(self) 
			...
			_G[db].enableDualProfile = not _G[db].enableDualProfile 
			db_variables[db]._profileMethode.args.spec2choose.disabled = not _G[db].enableDualProfile;
			
		end,
		get = function(self) return _G[db].enableDualProfile end,
	}
	
	f.args.spec2choose = {					
		name = L_DUAL_PROFILE, 
		desc = L_DUAL_PROFILE_DESC,
		disabled = not _G[db].enableDualProfile,
		order = 1.9,
		type = "dropdown",
