...
local function PrintSubElements(self, datas, maintree)
	...
	if datas.subelement then
		...
		for i, data in pairs(datas.subelement) do
			if data.show then
				self.id = self.id + 1
				C:GetTreeGroupElement(self, maintree..";"..data.name):UpdatePoint(self, data.realname, self.id, data.subelement and #data.subelement or 0, data.toggle)
...
function C:GetTreeGroupElement(parent, main)
	...
	local f = CreateFrame("Button", nil, parent.buttonParent)
...
function C:GetTreeGroupFramesFrame(parent)
	...
	local f = CreateFrame("ScrollFrame", nil, parent.leftSide)
	...
	--		print("UpdateSubElements", name, datas, datas.type, datas.embend)
			if datas.type == "group" and datas.embend ~= true then
				...
				s.subelement[#s.subelement+1] = { id = #s.subelement+1, name = name, order = datas.order, toggle = show, show = shown, realname = datas.name }
	...
	--	print("UpdateElements", data)
		...
		for name, datas in pairs(data.args) do
			...
			if datas.type == "group" then
				...
				s[#s+1] = { id = #s+1, name = name, order = datas.order, toggle = show, show = show, realname = datas.name }
		...
		--	print(i, data.show, data.name, data.order, data.subelement and #data.subelement or 0)
			...
			C:GetTreeGroupElement(self, data.name):UpdatePoint(self,data.realname, self.id, data.subelement and #data.subelement or 0, data.toggle)
