...
placeholder[GetSpellInfo(129197)] = GetSpellInfo(15407)
if GetSpellInfo(179338) then
placeholder[GetSpellInfo(179338)] = GetSpellInfo(48045)
end
...
local SetPing = function(self, value)
	...
	local left1 = self.parent:GetRight() or 0
	local left2 = self:GetRight() or 0
	
	if left2 >= left1 then
		self:SetWidth(mywidth-abs(left2-left1))
	end
...
local function CreateTicks(f)
	...
	tick:SetWidth(1)
	tick:SetHeight(h)
	tick:SetTexture(opts.tick_color[1],opts.tick_color[2],opts.tick_color[3],opts.tick_color[4])
	...
	lat3:SetTexture(opts.ping_color[1],opts.ping_color[2],opts.ping_color[3],opts.ping_color[4])
...
local function UpdateVisual(f)
	...
	f.bg:SetTexture(unpack(opts.color_bg))
	...
	f.icon.bg:SetTexture(unpack(opts.color_bg))
	...
	for i=1, #f.tiks do
		f.tiks[i]:SetTexture(unpack(opts.tick_color))
		f.tiks[i]:SetHeight(opts.h)
		
		f.tiks[i].latensy:SetTexture(opts.ping_color[1],opts.ping_color[2],opts.ping_color[3],opts.ping_color[4])
...
local function CreateCastBar(frame, w, h, drawticks)
	...
	f.ShowGCD = function(self, name)
		...
		local name = placeholder[name] or name
	...
	frame.CastBarUpdate = function(self)
		...
		self:UNIT_SPELLCAST_CHANNEL_START(_, self.unit)
		self:UNIT_SPELLCAST_START(_, self.unit)
	end
...
function C:GetCastBarGUI()
	...
	for k,v in ipairs(myCastBars) do
		...
		gui_option.args[v].args.ping = {
			...
			set = function(info,val) self.db.profile.castBars[v].ping = not self.db.profile.castBars[v].ping; self.UpdateSettings() end,
		...
		gui_option.args[v].args.target_name = {
			...
			set = function(info,val) self.db.profile.castBars[v].target_name = not self.db.profile.castBars[v].target_name; self.UpdateSettings() end,
		...
		gui_option.args[v].args.showGCD = {
			...
			set = function(info,val) self.db.profile.castBars[v].showGCD = not self.db.profile.castBars[v].showGCD; self.UpdateSettings() end,
...
local function OnEvent(self)
	...
	if IsSpellKnown(157223) then
		channeling_info[GetSpellInfo(15407)].amount = 4
		channeling_info[GetSpellInfo(15407)].every = 3/4
		channeling_info[GetSpellInfo(129197)].amount = 4
		channeling_info[GetSpellInfo(129197)].every = 3/4
	else
		channeling_info[GetSpellInfo(15407)].amount = 3
		channeling_info[GetSpellInfo(15407)].every = 1
		channeling_info[GetSpellInfo(129197)].amount = 3
		channeling_info[GetSpellInfo(129197)].every = 1
	end
end
