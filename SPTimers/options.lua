...
local function GetSpellOrItemID(val, tip)
	...
	if type(val) == "string" and not tonumber(val) then
		...
		local found, _, spellString = string.find(val, "^|c%x+|H(.+)|h%[.*%]")
		
		if found then
		
			local ctip, id = string.match(spellString, "(%a+):(%d+)")
...
local function GetClassSpec(spec)
	...
	if string.match(spec, CHOSEN_CLASS_SPEC) then return true end	
end
		...
		args = {
			...
			general={
				...
				args={
					...
					minimap = {
						order = 1,name = L["Show minimap"],type = "toggle",
					...
					global_style_settings = {
						...
						args = {
							...
							font_shadow_color = {
								order = 6.2,name = L["Shadow color"] ,type = "color", hasAlpha = true,
				...
				args={
							...
							--	width = "half",
								...
								values = function()
									...
									for k,v in pairs(self.db.profile.procSpells) do
										...
										if not v.deleted and not v.fulldel and ProcFilter(v.role) then
											local g_spellID, g_spellName  = IsGroupUpSpell(k)
					...
					spellList3 = {
						...
						args={
							...
							AddNew = {
								type = "editbox",	order	= 1,
								name = L["Spell ID"],
								desc = L["Change spellID"],
								
								dialogControl = "Spell_EditBox_SPTimer",
					  ...
					  spellList4 = {
						...
						args={							
							...
							AddNew = {
								...
								name = L["Spell ID"],
								desc = L["Change spellID"],
								dialogControl = "Player_EditBox_SPTimer",
							...
							AddNewSpellID = {
								...
								set = function(info,val)
									...
									if num then
										local spellname = GetSpellInfo(num)										
										if spellname then											
											self.db.profile.cooldownline.block[spellname] = { spellid = num, hide = true }
											
											blocklist_select = spellname
										end
									end	
								end,
								get = function(info) end
						...
						--		dialogControl = "Spell_EditBox_SPTimer",
								...
								--	print(num, tip, val)
									...
									if num then
										...
										if itemname then											
											self.db.profile.cooldownline.block[itemname] = { itemid = num, hide = true }
											
											blocklist_select = itemname
										end
									end	
								end,
								get = function(info) end
							...
							enabled = {
								...
								set = function(info,val) 
									if blocklist_select then
										self.db.profile.cooldownline.block[blocklist_select].hide = not self.db.profile.cooldownline.block[blocklist_select].hide
									end
								end,
								get = function(info)
							...
							--		print(blocklist_select)
									...
									return self.db.profile.cooldownline.block[blocklist_select] and self.db.profile.cooldownline.block[blocklist_select].hide or false
							...
							annonce = {
								...
								set = function(info,val) 
									if blocklist_select then 
										self.db.profile.cooldownline.block[blocklist_select].reporting  = not self.db.profile.cooldownline.block[blocklist_select].reporting 
									end
								end,
								get = function(info) 
									if not blocklist_select then return nil end
									return self.db.profile.cooldownline.block[blocklist_select] and self.db.profile.cooldownline.block[blocklist_select].reporting or false 
							...
							hide_splash = {
								...
								set = function(info,val)
									if blocklist_select then 
										self.db.profile.cooldownline.block[blocklist_select].hide_splash = not self.db.profile.cooldownline.block[blocklist_select].hide_splash 
									end
								end,
								get = function(info)
									if not blocklist_select then return nil end
									return self.db.profile.cooldownline.block[blocklist_select] and self.db.profile.cooldownline.block[blocklist_select].hide_splash or false
								end
							},
							SoundFile = {					
								type = "group",	order	= 6,
								...
								args = {						
									...
									--	dialogControl = 'LSM30_Sound',
										...
										set = function(info,value) 
											if blocklist_select then 
												self.db.profile.cooldownline.block[blocklist_select].sound_onshow = value
											end
										end,
										get = function(info) 
											if not blocklist_select then return "None" end
											return self.db.profile.cooldownline.block[blocklist_select] and self.db.profile.cooldownline.block[blocklist_select].sound_onshow or "None"; 
										end,
									},
									OnHide = {
									...
									--	dialogControl = 'LSM30_Sound',
										...
										set = function(info,value) 
											if blocklist_select then
												self.db.profile.cooldownline.block[blocklist_select].sound_onhide = value 
											end
										end,
										get = function(info) 
											if not blocklist_select then return "None" end
											return self.db.profile.cooldownline.block[blocklist_select] and self.db.profile.cooldownline.block[blocklist_select].sound_onhide or "None";
										end,
									},
								}
							},
							delete = {
								...
								desc = L['SemiDelDesc'],
								func = function(info, value)
									if self.db.profile.cooldownline.block[blocklist_select] then
										self.db.profile.cooldownline.block[blocklist_select].deleted = true
									end
							...
							fulldelete = {
								...
								func = function(info, value)
									if self.db.profile.cooldownline.block[blocklist_select] then
										self.db.profile.cooldownline.block[blocklist_select].fulldel = true
									end
									blocklist_select = nil								
								end,
							},
...
do
	...
	function C:InitSupports()
		...
		o.args.about.args.donations  = {
				...
				width = "full",
				set = function() end,
				get = function() return don_text end,
			}
	...
	function C:ClassSpell(spellID)
		...
		o.args.bars.args.ClassSpells.args.classGrop.args.hide = {
				order = 3.3,name = L["Hide"],type = "toggle", width = "full",
		...
		o.args.bars.args.ClassSpells.args.classGrop.args.cleu = {
				order = 4,name = L["cleu"], desc = L["cleu_desc"], type = "toggle",
				set = function(info,val) opts.cleu = not opts.cleu; end,
		...
		o.args.bars.args.ClassSpells.args.classGrop.args.tick = {
				type = "editbox",width = "half", 	order	= 7,
				name = L["Ticks"],
				desc = L["TicksComm"],
				set = function(info,val) 
					local num = tonumber(val)				
		...
		o.args.bars.args.ClassSpells.args.classGrop.args.cast = {
		...
		o.args.bars.args.ClassSpells.args.classGrop.args.withgcd = {
		...
		o.args.bars.args.ClassSpells.args.classGrop.args.tickoverlap = {
		...
		o.args.bars.args.ClassSpells.args.classGrop.args.CustomText.args.custom_text = {
				type = "editbox",	order	= 12, width = "full",
				name = L["Custom Text"],
		...
		o.args.bars.args.ClassSpells.args.classGrop.args.CustomText.args.custom_text_on= {
				order = 13,name = L["Custom Text On"],type = "toggle", width = "full",
		...
		o.args.bars.args.ClassSpells.args.classGrop.args.filterHeader.args.targetType = {
				name = L["Target Type"],
				order = 20.2,
				type = "dropdown",
				values = targetType,
		...
		o.args.bars.args.ClassSpells.args.anchor_per_unit = {		
			...
			args = {
					enabled = {
						...
						set = function(info,val) opts.anchor_per_unit_enabled = not opts.anchor_per_unit_enabled; end,
					...
					always_show_enabled = {
						...
						get = function(info) 						
							if not opts.unit_force_show then opts.unit_force_show = {} end					
							if unit_per_achor_choose then
								return opts.unit_force_show[unit_per_achor_choose]
							end
							return false
						end
					},
	...
	function C:ProcsSpell(spellID)
		...
		o.args.bars.args.spellList2.args.hide = {
				order = 3,name = L["Hide"],type = "toggle", width = "full",
				set = function(info,val) opts.hide = not opts.hide; end,
				get = function(info) return opts.hide end
			}			
		...
		o.args.bars.args.spellList2.args.customTextureGroup.args.custom_texture = {
				...
				set = function(info,val) 
					local num = tostring(val)				
					if num then opts.custom_texture = num end
				end,
				get = function(info) return opts.custom_texture and tostring(opts.custom_texture) or "" end
			}
		...
		o.args.bars.args.spellList2.args.filterHeader.args.targetType = {
				name = L["Target Type"],
				order = 20,
				type = "dropdown",
				values = targetType,
		...
		o.args.bars.args.spellList2.args.delete = {
			...
			func = function(info, value)
				...
				o.args.bars.args.spellList2.args.selectRole = {
					name = L["Role"],
					order = 1.1,
					desc = "Role",
					type = "dropdown",
	...
	function C:OthersSpell(spellID)
		...
		o.args.bars.args.spellList3.args.desc2.args.hide = {
			order = 9.1,name = L["Hide"],type = "toggle", width = "full",
			set = function(info,val) opts.hide = not opts.hide; end,
			get = function(info) return opts.hide end
		}
		...
		o.args.bars.args.spellList3.args.filterHeader.args.targetType = {
				name = L["Target Type"],
				order = 23,
				type = "dropdown",
				values = targetType,
...
function C:SetAnchorTable(value)
	...
	o.args.bars.args.style.args.StatusBarGroup.args.short_name = {
			...
			set = function(info,val) self.db.profile.bars_anchors[anchor_value].short = not self.db.profile.bars_anchors[anchor_value].short; self:UpdateFormatTexts(anchor_value) end,
	...
	o.args.bars.args.style.args.desc2.args.stack_font_alpha = {
		...
		o.args.bars.args.style.args.desc2.args.stack_shadow_color = {
	...
	o.args.bars.args.style.args.desc2.args.stack_shadow_offset_x = {
			name = L["Shadow offset X"],
			type = "slider",
			order	= 29.3,
			min		= -10,
	...
	o.args.bars.args.style.args.desc2.args.stack_shadow_offset_y = {
			name = L["Shadow offset Y"],
			type = "slider",
			order	= 29.4,
			min		= -10,
	...
	o.args.bars.args.style.args.desc3.args.timer_font_alpha = {
		...
		o.args.bars.args.style.args.desc3.args.timer_shadow_color = {
	...
	o.args.bars.args.style.args.desc3.args.timer_shadow_offset_x = {
			name = L["Shadow offset X"],
			type = "slider",
			order	= 35.3,
			min		= -10,
	...
	o.args.bars.args.style.args.desc3.args.timer_shadow_offset_y = {
			name = L["Shadow offset Y"],
			type = "slider",
			order	= 35.4,
			min		= -10,
	...
	o.args.bars.args.style.args.desc4.args.spell_text_offset = {
		...
		o.args.bars.args.style.args.desc4.args.spell_shadow_color = {
	...
	o.args.bars.args.style.args.desc4.args.spell_shadow_offset_x = {
			name = L["Shadow offset X"],
			type = "slider",
			order	= 41.4,
			min		= -10,
	...
	o.args.bars.args.style.args.desc4.args.spell_shadow_offset_y = {
			name = L["Shadow offset Y"],
			type = "slider",
			order	= 41.5,
			min		= -10,
	...
	o.args.bars.args.style.args.descRaidIcon.args.raidicon_x = {
			name = L["Offset X"],
			type = "slider",
			order	= 41.8,
			min		= -1000,
			...
			step	= 1,
			set = function(info,val) 
				self.db.profile.bars_anchors[anchor_value].raidicon_x = val
				self:UpdateRaidIcons()
			end,
			get = function(info)
				return	self.db.profile.bars_anchors[anchor_value].raidicon_x or 0
			end,
		}
	...
	o.args.bars.args.style.args.group_header_settings.args.textcolor = {
	...
	o.args.bars.args.style.args.group_header_settings.args.textcolorfocus = {
	...
	o.args.bars.args.style.args.group_header_settings.args.font = {
			order = 3,name = L["Font"],type = 'font',
		--	dialogControl = 'LSM30_Font',
			values = LSM:HashTable("font"), --LSM:HashTable("font"),
			set = function(info,key) 
	...
	o.args.bars.args.style.args.group_header_settings.args.size = {
			name = L["Size"],
			type = "slider",
			order	= 4,
			min		= 1,
	...
	o.args.bars.args.style.args.group_header_settings.args.justifu = {
	...
	o.args.bars.args.style.args.group_header_settings.args.flaggs = {
	...
	o.args.bars.args.style.args.group_header_settings.args.shadow_color = {
	...
	o.args.bars.args.style.args.group_header_settings.args.shadow_offset_x = {
			name = L["Shadow offset X"],
			type = "slider",
			order	= 9,
			min		= -10,
	...
	o.args.bars.args.style.args.group_header_settings.args.shadow_offset_y = {
			name = L["Shadow offset Y"],
			type = "slider",
			order	= 10,
			min		= -10,
...
function C:BarCooldownSpell(spellID)
	...
	o.args.bars.args.spellList4.args.header1.args.colorr = {
			order = 5,name = L["Color"],type = "color",
			set = function(info,r,g,b) opts.color ={r,g,b}; end,
			get = function(info) return opts.color and opts.color[1] or 0 , opts.color and opts.color[2] or 0, opts.color and opts.color[3] or 0 end,
		}

	o.args.bars.args.spellList4.args.header1.args.color_on = {
			order = 6,name = L["Own color"],type = "toggle",
			set = function(info,val) opts.color_on = not opts.color_on; end,
			get = function(info) return opts.color_on end
		}
